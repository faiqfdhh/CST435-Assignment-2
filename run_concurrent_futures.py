...
from filters import (process_single_image_futures, process_single_image_sequential_array,
                     process_images_pipeline_futures)
...
def run_benchmark_cf(func, image_arrays, num_workers=None, mode='seq'):
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("  CONCURRENT.FUTURES-ONLY BENCHMARK")
    print("  Using: ProcessPoolExecutor, executor.map, executor.submit")
    print("="*70 + "\n")
    
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    # Setup logging
    log_file = get_unique_filename(os.path.join(OUTPUT_DIR, 'benchmark_concurrent_futures.txt'))
    ...
    num_imgs = len(files)
    print_both(logger, "="*60)
    print_both(logger, "CONCURRENT.FUTURES BENCHMARK")
    print_both(logger, "="*60)
    ...
    print_both(logger, "-"*60)
    t_seq = run_benchmark_cf(process_single_image_sequential_array, image_arrays, mode='seq')
    ...
    # Step 2: Parallel Benchmarks (Concurrent.Futures Only)
    print_both(logger, "\n" + "-"*60)
    print_both(logger, "Step 2: Concurrent.Futures Parallel Benchmarks")
    print_both(logger, "-"*60)
    ...
    for w in worker_counts:
        ...
        print_both(logger, "\n[Pixel-level] ProcessPoolExecutor + submit...")
        t = run_benchmark_cf(process_single_image_futures, image_arrays, w, 'cf_pool')
        record_result(results, 'Pixel-Level', w, t, t_seq, logger)

        # Image-Level Parallelism
        print_both(logger, "\n[Image-level] executor.map...")
        t = run_benchmark_cf(process_single_image_sequential_array, image_arrays, w, 'cf_map')
        record_result(results, 'Image-Level', w, t, t_seq, logger)

        # Task-Level Parallelism
        print_both(logger, "\n[Task-level] futures with as_completed...")
        t = run_benchmark_cf(process_images_pipeline_futures, image_arrays, w, 'direct')
    ...
    # Export to CSV
    csv_file = get_unique_filename(os.path.join(OUTPUT_DIR, 'benchmark_concurrent_futures.csv'))
    ...
    # Export to JSON
    json_file = get_unique_filename(os.path.join(OUTPUT_DIR, 'benchmark_concurrent_futures.json'))
    ...
    ax.set_title('Speedup vs Workers (Concurrent.Futures)')
    ax.legend()
    ...
    plt.suptitle('CONCURRENT.FUTURES Benchmark Results', fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    chart_path = get_unique_filename(os.path.join(OUTPUT_DIR, 'performance_concurrent_futures.png'))
    plt.savefig(chart_path, dpi=150)
    print_both(logger, f"  - {chart_path}")
    
    print_both(logger, "\n" + "="*60)
    print_both(logger, "CONCURRENT.FUTURES BENCHMARK COMPLETE")
    print_both(logger, "="*60)
